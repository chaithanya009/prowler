...
from prowler.providers.okta.services.policy.policy_service import (
    ...
    legacy_email_recovery_enabled,
)


class policy_account_management_recovery_enabled(Check):
    """Ensure recovery is governed by Okta account management policy."""
    ...
    def execute(self) -> list[CheckReportOkta]:
        ...
        if _has_active_account_management_policy():
            ...
            report.status_extended = (
                "Okta account management policy governs password recovery."
            )
            return [report]

        if _has_legacy_email_recovery():
            report.status = "FAIL"
            report.status_extended = (
                "Okta password recovery relies on legacy email recovery instead of "
...
def _has_legacy_email_recovery() -> bool:
    for policy in policy_client.password_policies.values():
        if policy.status == "ACTIVE" and legacy_email_recovery_enabled(policy):
            return True
    return False
